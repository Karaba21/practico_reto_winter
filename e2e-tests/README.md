# E2E Tests - Operación Frazadita

Pruebas E2E con **Behave** (Gherkin) + **Playwright** (Python), usando el patrón
**Page Object Model**. Simulan el flujo de compra de un usuario buscando
productos de invierno en [geant.com.uy](https://www.geant.com.uy/), agregando
al menos dos productos distintos al carrito y validando que los ítems y el
total sean correctos.
...
> `environment.py` vive dentro de `features/` porque Behave sólo lo autodetecta
> ahí (es su convención fija, no una carpeta arbitraria).
...
carrito cambia, los selectores a ajustar son los de `features/pages/cart_page.py`.
